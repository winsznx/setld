...
| S15 | Submission integrity + judge path | PARTIAL | 2026-09-04 | hosted GitHub Actions GREEN on all 4 jobs from a genuinely fresh checkout (forge-std pinned as git submodule, single pnpm source of truth) — latest run e07a9d7. README/WHITEPAPER/facts/claims/GATES synced to the hardened deployment; the superseded chain ID appears only in DECISIONS history (CI-guarded). Canonical-commit freeze + recorded demo pending S11 wide + transacting e2e (both gated on Attestcoin attestation throughput). |
